    ...
    */
    ...
    DFA.prototype.cumsumVariance = function (x, mean) {
        let cumulativeSumVariance = (sum => value => sum += (value - mean))(0)
    ...
    DFA.prototype.RMS = function (cut, fit) {
        if (cut.length != fit.length) {
            throw new Error('for calculating the RMS of the two vectors they should be equal');
        }
    ...
    DFA.prototype.compute = function (min_window = 4, step = 0.5) {
        ...
        */
        ...
        for (let s in scales) {
            ...
            split_scales[window_size] = this.splitArrayIntoChunks(cumsum, window_size, true)
